...
library DistributionMath {
    ...
    function calculateRequiredCollateral(
        ...
        // Generate hint only if provided hint is zero
        if (hint == 0) {
            if (from_mu < to_mu) {
                // Moving up in price, minimum will be above to_mu
                hint = to_mu + int256(to_sigma);
            } else {
                // Moving down in price, minimum will be below to_mu
                hint = to_mu - int256(to_sigma);
            }
        }
        
        // Use standard iteration and tolerance values
        uint256 max_iterations = 20;
        uint256 tolerance = 1000000000000;  // 1e-6 when scaled
        
        (uint256 maxLoss, ) = findMaximumLoss(
            from_mu,
            from_sigma,
            to_mu,
            to_sigma,
